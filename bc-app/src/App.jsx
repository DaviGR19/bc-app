...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",background:C.white,padding:"0 28px" }}>
      ...
      <Inp label="E-mail" type="email" placeholder="seu.nome@uel.br" value={email}
      ...
      <Btn full onClick={handle} disabled={loading}>{loading?"Entrando...":"Entrar"}</Btn>
    </div>
  );
}
...
function ProfileScreen({ user, onUpdate, onLogout, onBack }) {
  ...
  async function handleAvatar(e) {
    ...
    onUpdate({ avatar: data.publicUrl });
  }
...
function DemandaDetail({ demanda, canEdit, onUpdate, onDelete, onBack }) {
  ...
  useEffect(()=>{
    supabase.from("anexos").select("*").eq("demanda_id",demanda.id)
  ...
  async function save() {
    const { data } = await supabase.from("demandas").update({
      ...
      resp:form.resp, prazo:form.prazo, status:form.status,
                ...
                <div key={a.id}>
                  <div style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 0" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:11,color:C.muted }}>{a.tamanho}</div>
                    </div>
                    <a href={a.url} target="_blank" rel="noreferrer" style={{ width:30,height:30,borderRadius:7,border:`1px solid ${C.border}`,background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",textDecoration:"none" }}>
                      {Ic.down()}
                    </a>
                  </div>
...
function FullCalendar({ eventos, canManage, onAdd }) {
  ...
  async function add(){
    ...
    onAdd(data);
    setNovo({titulo:"",date:"",hora:"",local:""}); setForm(false);
  }
...
export default function BCApp() {
  ...
  // Carrega dados quando faz login
  useEffect(()=>{
    if(!user) return;
  ...
  async function logout(){
    ...
    setUser(null); setScreen("app"); setTab("inicio");
  }
  ...
  if(!user) return <LoginScreen onLogin={u=>{setUser(u);setTab("inicio");setScreen("app");}}/>;
  ...
  if(screen==="demanda"&&selectedD){
    ...
    if(d) return (
      <DemandaDetail demanda={d} canEdit={canManage(user)}
        onUpdate={updated=>setDemandas(prev=>prev.map(x=>x.id===updated.id?updated:x))}
        onDelete={async()=>{ await supabase.from("demandas").delete().eq("id",d.id); setDemandas(prev=>prev.filter(x=>x.id!==d.id)); setScreen("app"); setSelectedD(null); }}
  ...
  const nextEventos = eventos.filter(e=>( e.data||e.date)>=todayIso());
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",color:C.text }}>
      ...
      <div style={{ paddingBottom:88 }}>
        ...
        {/* CALENDÁRIO */}
        {tab==="calendario"&&<FullCalendar eventos={eventos} canManage={canManage(user)} onAdd={ev=>setEventos(prev=>[...prev,ev])}/>}
