...
function Inp({ label, ...p }) {
  return <div style={{ marginBottom:12 }}>{label&&<FL>{label}</FL>}<input {...p} style={{ width:"100%",padding:"11px 12px",borderRadius:9,border:`1.5px solid ${C.border}`,fontSize:14,color:C.text,background:C.bg,fontFamily:"inherit",boxSizing:"border-box",...(p.style||{}) }}/></div>;
}
function Sel({ label, children, ...p }) {
  return <div style={{ marginBottom:12 }}>{label&&<FL>{label}</FL>}<select {...p} style={{ width:"100%",padding:"11px 12px",borderRadius:9,border:`1.5px solid ${C.border}`,fontSize:14,color:C.text,background:C.bg,fontFamily:"inherit" }}>{children}</select></div>;
}
function Txt({ label, ...p }) {
  return <div style={{ marginBottom:12 }}>{label&&<FL>{label}</FL>}<textarea {...p} style={{ width:"100%",padding:"11px 12px",borderRadius:9,border:`1.5px solid ${C.border}`,fontSize:14,color:C.text,background:C.bg,fontFamily:"inherit",resize:"vertical",boxSizing:"border-box",...(p.style||{}) }}/></div>;
}
...
const Ic = {
  ...
  logout: c=><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={c||C.red} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>,
  camera: c=><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={c||C.white} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>,
  ...
  members:()=><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.blue} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>,
  image:  c=><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={c||C.white} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>,
};

// ── Image Cropper (estilo WhatsApp) ───────────────────────────
function ImageCropper({ src, aspect, onCrop, onCancel }) {
  ...
  const PREVIEW_W = 300;
  const PREVIEW_H = isSquare ? 300 : 169; // 16:9 para banner
  ...
  function onStart(e) {
    e.preventDefault();
    setDragging(true);
    setLastPos(getPoint(e));
  }
  function onMove(e) {
    if (!dragging) return;
    e.preventDefault();
    const p = getPoint(e);
    setOffsetX(x => x + p.x - lastPos.x);
    setOffsetY(y => y + p.y - lastPos.y);
    setLastPos(p);
  }
  ...
  function handleCrop() {
    ...
    const OUT_W = isSquare ? 400 : 800;
    const OUT_H = isSquare ? 400 : 450;
    canvas.width = OUT_W;
    canvas.height = OUT_H;
    ...
    const left = (PREVIEW_W - dispW) / 2 + offsetX;
    const top  = (PREVIEW_H - dispH) / 2 + offsetY;

    ctx.drawImage(
      img,
      (-left / scale) * scaleX,
      (-top  / scale) * scaleY,
      (PREVIEW_W / scale) * scaleX,
      (PREVIEW_H / scale) * scaleY,
      0, 0, OUT_W, OUT_H
    );
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div style={{ background:C.white,borderRadius:18,padding:20,width:"100%",maxWidth:360 }}>
        <div style={{ fontSize:16,fontWeight:700,marginBottom:4 }}>Ajustar foto</div>
        <div style={{ fontSize:12,color:C.sub,marginBottom:14 }}>Arraste para reposicionar · Deslize para zoom</div>

        {/* Preview area */}
        <div ref={containerRef} style={{ width:PREVIEW_W,height:PREVIEW_H,borderRadius:isSquare?PREVIEW_W:12,overflow:"hidden",background:"#eee",margin:"0 auto 16px",cursor:"grab",position:"relative",userSelect:"none" }}
...
function AuthScreen({ onLogin }) {
  ...
  async function handleSignup() {
    ...
    if(pw.length<6){setError("Senha precisa ter pelo menos 6 caracteres.");return;}
    setError("");setLoading(true);
    ...
    await supabase.from("profiles").insert({id:data.user.id,name:name.trim(),initials,area:"Gestão de Pessoas",role:"membro",bio:"",avatar:"",bannerColor:C.blue,bannerImg:""});
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",background:C.white,padding:"0 28px",fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <div style={{ textAlign:"center",marginBottom:36 }}>
        ...
        <div style={{ fontSize:13,color:C.sub }}>{mode==="login"?"Entre com seu e-mail institucional":"Use seu e-mail @uel.br para se cadastrar"}</div>
      </div>
      ...
      <button onClick={()=>{setMode(mode==="login"?"signup":"login");setError("");setSuccess("");}} style={{ marginTop:16,background:"none",border:"none",cursor:"pointer",fontSize:13,color:C.blue,fontWeight:600 }}>
...
function ProfileScreen({ user, onUpdate, onLogout, onBack }) {
  ...
  const [saved,setSaved]=useState(false); const [pwErr,setPwErr]=useState("");
  const [loading,setLoading]=useState(false);
  ...
  function onFileChange(e, aspect) {
    ...
    const r=new FileReader(); r.onload=ev=>{setCropSrc(ev.target.result);setCropAspect(aspect);}; r.readAsDataURL(f);
    e.target.value="";
  }
  ...
  async function saveBannerColor(c) {
    setBannerColor(c);
    await supabase.from("profiles").update({bannerColor:c,bannerImg:""}).eq("id",user.id);
    onUpdate({bannerColor:c,bannerImg:""});
  }
  ...
  async function handleSave() {
    ...
    onUpdate({name,bio,initials}); setPw("");setPw2("");setLoading(false);
    setSaved(true); setTimeout(()=>setSaved(false),2500);
  }

  const bannerStyle = user.bannerImg
    ? {backgroundImage:`url(${user.bannerImg})`,backgroundSize:"cover",backgroundPosition:"center"}
    : {background:bannerColor};

  return (
    <div style={{ minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&display=swap'); *{box-sizing:border-box;} input::placeholder,textarea::placeholder{color:#9CA3AF;} input:focus,textarea:focus{outline:none;border-color:${C.blue}!important;} button{font-family:inherit;} select{font-family:inherit;}`}</style>
      {cropSrc&&<ImageCropper src={cropSrc} aspect={cropAspect} onCrop={onCropDone} onCancel={()=>setCropSrc(null)}/>}
      ...
      {/* Banner */}
      <div style={{ ...bannerStyle,height:140,position:"relative" }}>
        <button onClick={()=>pickFile(bannerRef,"banner")}
          style={{ position:"absolute",bottom:10,right:10,padding:"6px 12px",borderRadius:8,background:"rgba(0,0,0,0.45)",border:"1.5px solid rgba(255,255,255,0.4)",cursor:"pointer",display:"flex",alignItems:"center",gap:6,color:"white",fontWeight:600,fontSize:12,fontFamily:"inherit" }}>
          {Ic.image()} Alterar fundo
        </button>
      </div>

      {/* Avatar over banner */}
      <div style={{ display:"flex",alignItems:"flex-end",padding:"0 20px",marginTop:-40,marginBottom:16,position:"relative",zIndex:1 }}>
        <div style={{ position:"relative" }}>
          <Avatar user={user} size={80}/>
          <button onClick={()=>pickFile(avatarRef,"square")}
            style={{ position:"absolute",bottom:0,right:0,width:26,height:26,borderRadius:"50%",background:"rgba(0,0,0,0.5)",border:"2px solid white",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>
            {Ic.camera()}
          </button>
        </div>
        <div style={{ marginLeft:12,paddingBottom:4 }}>
          <div style={{ fontWeight:700,fontSize:16 }}>{user.name}</div>
          <div style={{ fontSize:12,color:C.sub }}>{getRoleLabel(user.role)} · {user.area}</div>
        </div>
      </div>

      <div style={{ margin:"0 16px",background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:20 }}>
        {/* Cor do banner */}
        {!user.bannerImg&&(
          <div style={{ marginBottom:16 }}>
            <FL>Cor do fundo</FL>
            <div style={{ display:"flex",gap:10 }}>
              {BG_COLORS.map(c=>(
                <button key={c} onClick={()=>saveBannerColor(c)} style={{ width:30,height:30,borderRadius:"50%",background:c,border:bannerColor===c?`3px solid ${C.text}`:`3px solid transparent`,cursor:"pointer",flexShrink:0 }}/>
              ))}
            </div>
          </div>
        )}
        {user.bannerImg&&(
          <button onClick={()=>saveBannerColor(bannerColor)} style={{ marginBottom:16,fontSize:12,color:C.red,background:"#FEF2F2",border:"none",padding:"6px 12px",borderRadius:8,cursor:"pointer",fontWeight:600,fontFamily:"inherit" }}>
...
function DemandaDetail({ demanda, canEdit, onUpdate, onDelete, onBack }) {
  ...
  async function handleAnexo(e) {
    ...
    const tam=f.size>1048576?`${(f.size/1048576).toFixed(1)} MB`:`${Math.round(f.size/1024)} KB`;
  ...
  return (
    <div style={{ minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&display=swap'); *{box-sizing:border-box;} input::placeholder,textarea::placeholder{color:#9CA3AF;} input:focus,textarea:focus,select:focus{outline:none;border-color:${C.blue}!important;} button{font-family:inherit;} select{font-family:inherit;}`}</style>
...
function NavTab({ active, icon, label, onClick }) {
  return (
    <button onClick={onClick} style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:"none",border:"none",cursor:"pointer",padding:"4px 12px",transition:"transform 0.15s",transform:active?"translateY(-2px)":"translateY(0)" }}>
      <div style={{ width:40,height:32,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",background:active?C.blueSoft:"transparent",transition:"background 0.2s" }}>{icon(active?C.blue:C.muted)}</div>
      <span style={{ fontSize:10,fontWeight:700,color:active?C.blue:C.muted,letterSpacing:0.3 }}>{label}</span>
    </button>
  );
}
...
export default function BCApp() {
  ...
  const [showMembers,setShowMembers]=useState(false); // membros dentro do drawer
  ...
  async function loadMateriais(){
    ...
    const g={};AREAS.forEach(a=>g[a]=[]);
    (data||[]).forEach(m=>{if(g[m.area])g[m.area].push(m);});
    setMateriais(g);
  }
  ...
  async function handleUpload(e){
    ...
    const ext=f.name.split(".").pop().toUpperCase();
    const tam=f.size>1048576?`${(f.size/1048576).toFixed(1)} MB`:`${Math.round(f.size/1024)} KB`;
    await supabase.from("materiais").insert({nome:f.name,tipo:ext,tamanho:tam,area:uploadArea,url:url.publicUrl,criado_por:user.id});
  ...
  if(loading) return <div style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.white,fontFamily:"'DM Sans',sans-serif" }}><Spinner/></div>;
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",color:C.text,position:"relative",overflow:"hidden" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&display=swap'); *{box-sizing:border-box;} input::placeholder,textarea::placeholder{color:#9CA3AF;} input:focus,textarea:focus,select:focus{outline:none;border-color:${C.blue}!important;} button{font-family:inherit;} ::-webkit-scrollbar{display:none;} select{font-family:inherit;} @keyframes slideInRight{from{transform:translateX(100%)}to{transform:translateX(0)}} @keyframes fadeIn{from{opacity:0}to{opacity:1}}`}</style>
      <input ref={uploadRef} type="file" style={{display:"none"}} onChange={handleUpload}/>

      {/* ── DRAWER — desliza por cima do conteúdo ── */}
      {drawer&&(
        <div style={{ position:"absolute",inset:0,zIndex:100,display:"flex" }}>
          {/* backdrop clicável */}
          <div onClick={()=>{setDrawer(false);setShowMembers(false);}} style={{ flex:1,background:"rgba(0,0,0,0.4)",animation:"fadeIn 0.2s ease" }}/>
          {/* painel lateral */}
          <div style={{ width:280,background:C.white,height:"100%",overflowY:"auto",display:"flex",flexDirection:"column",animation:"slideInRight 0.25s ease",boxShadow:"-4px 0 20px rgba(0,0,0,0.15)" }}>
            ...
            {!showMembers ? (
              ...
              <>
                <div style={{ background:C.blue,padding:"36px 20px 24px" }}>
                  ...
                  <div style={{ color:"rgba(255,255,255,0.75)",fontSize:12,marginTop:4 }}>{getRoleLabel(user.role)} · {user.area}</div>
                </div>
                <div style={{ flex:1 }}>
                  {[
                    { label:"Meu Perfil", sub:"Editar informações e foto", action:()=>{setDrawer(false);setScreen("profile");},
                      icon:<div style={{ width:38,height:38,borderRadius:10,background:C.blueSoft,display:"flex",alignItems:"center",justifyContent:"center" }}>{Ic.person(C.blue)}</div> },
                    { label:"Membros", sub:"Ver todos os participantes", action:()=>{setShowMembers(true);if(!membersLoaded)loadMembers();},
                      icon:<div style={{ width:38,height:38,borderRadius:10,background:C.blueSoft,display:"flex",alignItems:"center",justifyContent:"center" }}>{Ic.members()}</div> },
                  ].map((item,i)=>(
                    <div key={i}>
                      <button onClick={item.action} style={{ width:"100%",display:"flex",alignItems:"center",gap:14,padding:"16px 20px",background:"none",border:"none",cursor:"pointer",textAlign:"left" }}>
                        {item.icon}
                        <div style={{ flex:1 }}>
                ...
                <div style={{ padding:"16px 20px 40px" }}>
                  <button onClick={logout} style={{ width:"100%",display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"#FEF2F2",border:"none",borderRadius:10,cursor:"pointer",fontFamily:"inherit" }}>
                    {Ic.logout()}<span style={{ fontSize:14,fontWeight:600,color:C.red }}>Sair da conta</span>
                  </button>
                </div>
              </>
            ) : (
              /* ── Tela de membros dentro do drawer ── */
              <>
                <div style={{ background:C.white,borderBottom:`1px solid ${C.border}`,padding:"14px 16px",display:"flex",alignItems:"center",gap:10,position:"sticky",top:0 }}>
                  <button onClick={()=>setShowMembers(false)} style={{ background:"none",border:"none",cursor:"pointer",padding:4,display:"flex" }}>{Ic.back()}</button>
                  <span style={{ fontSize:16,fontWeight:700 }}>Membros</span>
                </div>
                {!membersLoaded&&<Spinner/>}
                {["presidente","vice","diretor","membro"].map(role=>{
                  const group=members.filter(m=>m.role===role);
                  if(!group.length)return null;
                  const rl={presidente:"Presidência",vice:"Vice",diretor:"Diretores",membro:"Assessores"};
                  return (
                    <div key={role}>
                      <div style={{ padding:"14px 16px 6px" }}>
                        <span style={{ fontSize:10,fontWeight:700,letterSpacing:1.5,color:C.muted,textTransform:"uppercase" }}>{rl[role]}</span>
                      </div>
                      {group.map((m,i)=>(
                        <div key={m.id}>
                          <div style={{ padding:"10px 16px" }}>
                            <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:user.role==="presidente"&&m.id!==user.id?8:0 }}>
                              ...
                              <div style={{ flex:1,minWidth:0 }}>
                                ...
                                <div style={{ fontSize:11,color:C.sub }}>{m.area}</div>
                              </div>
                            </div>
                            {user.role==="presidente"&&m.id!==user.id&&(
                              <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                                <select defaultValue={m.role} onChange={e=>updateMemberRole(m.id,e.target.value,m.area)}
                                  style={{ width:"100%",padding:"6px 8px",borderRadius:7,border:`1px solid ${C.border}`,fontSize:12,color:C.text,background:C.bg,fontFamily:"inherit" }}>
        ...
        {tab==="inicio"&&(<>
          ...
          {urgentes.length===0&&nextEventos.length===0&&(
            <div style={{ margin:"32px 20px",background:C.white,border:`1px solid ${C.border}`,borderRadius:16,padding:"32px 20px",textAlign:"center" }}>
              ...
              <div style={{ fontSize:15,fontWeight:600,marginBottom:6 }}>Tudo em dia!</div>
              ...
              <div key={area}>
                ...
                {isOpen&&(
                  <div style={{ margin:"0 20px 6px" }}>
                    {canUp&&<button onClick={()=>{setUploadArea(area);setTimeout(()=>uploadRef.current.click(),50);}} style={{ width:"100%",padding:"10px 14px",marginBottom:8,borderRadius:10,border:`1.5px dashed ${C.blue}`,background:C.blueSoft,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,color:C.blue,fontWeight:600,fontSize:13,fontFamily:"inherit" }}>{Ic.upload(C.blue)} Fazer upload de arquivo</button>}
                    {files.length>0&&(
                      <div style={{ background:C.white,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden" }}>
                        {files.map((f,fi)=>(
                          <div key={f.id}>
                            <div style={{ padding:"11px 14px",display:"flex",alignItems:"center",gap:12 }}>
                              <div style={{ minWidth:36,height:36,borderRadius:8,background:C.blueSoft,display:"flex",alignItems:"center",justifyContent:"center" }}><span style={{ fontSize:9,fontWeight:800,color:C.blue,letterSpacing:0.5 }}>{f.tipo}</span></div>
      ...
      {/* Bottom Nav */}
      <div style={{ position:"sticky",bottom:0,zIndex:40,background:C.white,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-around",padding:"8px 0 18px" }}>
